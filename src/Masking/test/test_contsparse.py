...
def test_conv1d_init():
    mask_fill_val = 19.0
    layer = ContSparseConv1d(1, 10, 3, bias=True, ablation="none", mask_bias=True, mask_init_value=mask_fill_val)
    ...
    layer = ContSparseConv1d(1, 10, 3, bias=True, ablation="none", mask_bias=False, mask_init_value=mask_fill_val)
    ...
    layer = ContSparseConv1d(1, 10, 3, bias=False, ablation="none", mask_bias=False, mask_init_value=mask_fill_val)
    ...
    # Assert that you cannot mask a bias that isn't there
    with pytest.raises(Exception):
        layer = ContSparseConv1d(1, 10, 3, bias=False, ablation="none", mask_bias=True, mask_init_value=mask_fill_val)

def test_conv1d_from_layer():
    base_layer = nn.Conv1d(1, 10, 3)
    masked_layer = ContSparseConv1d.from_layer(base_layer, ablation="none", mask_bias=True, mask_init_value=1.0)
...
def test_conv1d_zero_ablate():
    layer = ContSparseConv1d(1, 10, 3, bias=True, ablation="zero_ablate", mask_bias=False, mask_init_value=0.0)
    ipt = torch.ones(1, 10)
    layer.weight_mask_params = nn.Parameter(torch.rand(layer.weight_mask_params.size()) - .5)
    layer.train(False)
    _ = layer(ipt) # Weight mask computed during forward pass
...
def test_conv1d_random_ablate():
    layer = ContSparseConv1d(1, 10, 3, bias=True, ablation="random_ablate", mask_bias=False, mask_init_value=0.0)
    ipt = torch.ones(1, 10)
    layer.weight_mask_params = nn.Parameter(torch.rand(layer.weight_mask_params.size()) - .5)
    layer.train(False)
    rand_out = layer(ipt) # Weight mask computed during forward pass
...
def test_conv1d_sampled_ablate():
    layer = ContSparseConv1d(1, 10, 3, bias=True, ablation="randomly_sampled", mask_bias=False, mask_init_value=0.0)
    ipt = torch.ones(1, 10)
    layer.weight_mask_params = nn.Parameter(torch.rand(layer.weight_mask_params.size()) - .7)
    layer.train(False)
    _ = layer(ipt) # Weight mask computed during forward pass
    ...
    # Assert that an error is thrown if the subnetwork params contain >50% of parameters
    layer = ContSparseConv1d(1, 10, 3, bias=True, ablation="randomly_sampled", mask_bias=False, mask_init_value=0.0)
    layer.weight_mask_params = nn.Parameter(torch.rand(layer.weight_mask_params.size()) - .1)
    layer.train(False)
    with pytest.raises(Exception):
        _ = layer(ipt) # Weight mask computed during forward pass
...
def test_conv1d_masking():
    # Test that masking every param gives the zero vector
    layer = ContSparseConv1d(1, 10, 3, bias=True, ablation="none", mask_bias=True, mask_init_value=-0.1)
    ipt = torch.ones(1,10)
    layer.train(False)
    out = layer(ipt)
    assert torch.all(out == torch.zeros(out.shape))
    ...
    # Test that masking every weight param gives the bias vector
    layer = ContSparseConv1d(1, 10, 3, bias=True, ablation="none", mask_bias=False, mask_init_value=-0.1)
    ipt = torch.ones(1, 10)
    layer.train(False)
    out = layer(ipt)
    ...
    layer = ContSparseConv1d(1, 10, 3, bias=True, ablation="none", mask_bias=True, mask_init_value=1.0)
...
def test_conv2d_init():
    mask_fill_val = 19.0
    layer = ContSparseConv2d(1, 10, 3, bias=True, ablation="none", mask_bias=True, mask_init_value=mask_fill_val)
    ...
    layer = ContSparseConv2d(1, 10, 3, bias=True, ablation="none", mask_bias=False, mask_init_value=mask_fill_val)
    ...
    layer = ContSparseConv2d(1, 10, 3, bias=False, ablation="none", mask_bias=False, mask_init_value=mask_fill_val)
    ...
    # Assert that you cannot mask a bias that isn't there
    with pytest.raises(Exception):
        layer = ContSparseConv2d(1, 10, 3, bias=False, ablation="none", mask_bias=True, mask_init_value=mask_fill_val)

def test_conv2d_from_layer():
    base_layer = nn.Conv2d(1, 10, 3)
    masked_layer = ContSparseConv2d.from_layer(base_layer, ablation="none", mask_bias=True, mask_init_value=1.0)
...
def test_conv2d_zero_ablate():
    layer = ContSparseConv2d(1, 10, 3, bias=True, ablation="zero_ablate", mask_bias=False, mask_init_value=0.0)
...
def test_conv2d_random_ablate():
    layer = ContSparseConv2d(1, 10, 3, bias=True, ablation="random_ablate", mask_bias=False, mask_init_value=0.0)
    ipt = torch.ones(1, 10, 10)
    layer.weight_mask_params = nn.Parameter(torch.rand(layer.weight_mask_params.size()) - .5)
    layer.train(False)
    rand_out = layer(ipt) # Weight mask computed during forward pass
...
def test_conv2d_sampled_ablate():
    layer = ContSparseConv2d(1, 10, 3, bias=True, ablation="randomly_sampled", mask_bias=False, mask_init_value=0.0)
    ipt = torch.ones(1, 10, 10)
    layer.weight_mask_params = nn.Parameter(torch.rand(layer.weight_mask_params.size()) - .7)
    layer.train(False)
    _ = layer(ipt) # Weight mask computed during forward pass
...
def test_conv2d_masking():
    # Test that masking every param gives the zero vector
    layer = ContSparseConv2d(1, 10, 3, bias=True, ablation="none", mask_bias=True, mask_init_value=-0.1)
    ipt = torch.ones(1, 10, 10)
    layer.train(False)
    out = layer(ipt)
    assert torch.all(out == torch.zeros(out.shape))
    ...
    # Test that masking every weight param gives the bias vector
    layer = ContSparseConv2d(1, 10, 3, bias=True, ablation="none", mask_bias=False, mask_init_value=-0.1)
    layer.train(False)
    out = layer(ipt)
    assert torch.all(out[:, 0, 0] == layer.bias)

    # Test that masking one filter gives a particular zero entry
    layer = ContSparseConv2d(1, 10, 3, bias=True, ablation="none", mask_bias=True, mask_init_value=1.0)
