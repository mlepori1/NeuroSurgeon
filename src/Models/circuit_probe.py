...
class CircuitProbe(PretrainedModel):
    ...
    def __init__(self, config, model, tokenizer):
        ...
        self.wrapped_model = CircuitModel(self._config_2_circuit_model_config(config), model)
        self.wrapped_model = ResidualUpdateModel(self._config_2_residual_update_config(config), self.wrapped_model)
    ...
    def forward(self, input_ids=None, return_dict=True, **kwargs):
        ...
        not_subword = torch.stack([torch.tensor( [ not tok.startswith(self.config.subword_identifer) for tok in self.tokenizer.convert_ids_to_tokens(ids) ] ) for ids in input_ids])
        ...
        if self.config.rm_loss == "soft_NN":
            ...
            identity_matrix = ~torch.eye(concept_rsm.shape[0], dtype=torch.bool, device=concept_rsm.device) 
            ...
            EPSILON = 1E-5
            numerator = torch.sum(torch.exp(-rsm) * numerator_pairs, dim=1) + EPSILON # If no same class pairs, this is equivalent to not computing loss over that class
